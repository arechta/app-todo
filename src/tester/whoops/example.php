...
class ExceptionTesting extends BaseException
{
	function getSomeSpecificData() {
		return 'Data';
	}
}
...
$handler->addDataTableCallback('Details', function (\Whoops\Exception\Inspector $inspector) {
	...
	if ($exception instanceof ExceptionTesting) {
...
function fooBar()
{
	throw new ExceptionTesting("Something broke!");
}

function bar()
{
	whoops_add_stack_frame(function () {
		fooBar('testing', 'asdas');
	});
}
