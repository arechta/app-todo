...
$thisPath = dirname(__FILE__).'/configs';
...
function put($route, $path_to_include) {
	if(strtoupper(trim($_SERVER['REQUEST_METHOD'])) === 'PUT') {
		route($route, $path_to_include);
	}
}
function patch($route, $path_to_include) {
	if(strtoupper(trim($_SERVER['REQUEST_METHOD'])) === 'PATCH') {
		route($route, $path_to_include);
	}
}
...
function route($route, $path_to_include) {
	...
	$projectPath  = str_replace($hostName, '', getURI(2));

	$request_url = filter_var($_SERVER['REQUEST_URI'], FILTER_SANITIZE_URL);
	$request_url = rtrim($request_url, '/');
	$request_url = strtok($request_url, '?');
	$request_url = str_replace($projectPath, '', $request_url);
